...
newtype S2CellId = S2CellId { cid :: Word64 }
...
getFace S2CellId { cid } = Face $ fromIntegral $ cid `shiftR` posBits


isValid :: S2CellId -> Bool
isValid i = unFace (getFace i) < numFaces && (lsb i .&. 0x1555555555555555 /= 0)
...
lsb :: S2CellId -> Word64
lsb S2CellId { cid } = cid .&. (-cid)


-- | Return the lowest-numbered bit that is on for cells at the given level.
lsbForLevel :: Level -> Word64
lsbForLevel (Level level) =
  1 `shiftL` (2 * (S2.maxCellLevel - level))


parent :: S2CellId -> S2CellId
parent i =
  let newLsb = lsb i `shiftL` 2 in
  S2CellId ((cid i .&. (-newLsb)) .|. newLsb);


-- | Return the cell at the previous level or at the given level (which must be
-- less than or equal to the current level).
parentForLevel :: Level -> S2CellId -> S2CellId
parentForLevel level i =
  let newLsb = lsbForLevel level in
  S2CellId ((cid i .&. (-newLsb)) .|. newLsb);
...
fromFacePosLevel (Face face) pos level =
  parentForLevel level $ S2CellId $ ((fromIntegral face :: Word64) `shiftL` posBits) + (pos .|. 1)
